...
export default function AdminLayout() {
  ...
  const sidebar = (
    <aside className="w-64 shrink-0 bg-ocean-950 text-ocean-100 flex flex-col h-full">
              ...
              {g.items.map((it) => (
                <li key={it.to}>
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      clsx("flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition", isActive ? "bg-white/10 text-white font-medium" : "text-ocean-200 hover:bg-white/5 hover:text-white")
                    }
                  >
  ...
  return (
    <div className="min-h-screen flex bg-slate-50">
      ...
      <div className="flex-1 min-w-0">
        <header className="lg:hidden h-14 flex items-center gap-3 px-4 bg-white border-b border-slate-100 sticky top-0 z-40">
          <button onClick={() => setOpen(true)} className="btn-ghost p-2">
            <Menu className="w-5 h-5" />
          </button>
          <span className="font-display font-semibold">Riding Factory</span>
        </header>
